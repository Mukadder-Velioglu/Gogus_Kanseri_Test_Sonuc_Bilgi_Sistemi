...
if(isset($_POST['eposta']) and isset($_POST['sifre'])){
	...
	$kullanici=$database->get("kullanicilar","*",["AND"=>["eposta"=>$eposta,"sifre"=>$sifre, "gorev"=>["Laboratuvar"]]]);

	if($kullanici["id"]>0){
		$_SESSION["id"]=$kullanici["id"];
		header("Location:laboratuvarekran.php");
	}else{
		header("Location:laboratuvar.php?mesaj=hata2");
	}
}else{
